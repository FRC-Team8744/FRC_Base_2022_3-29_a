...
public class HookArm extends SubsystemBase {
  private static final int deviceID = 20;  // Move to Constants once set in hardware!
  private static final boolean debugHookArm = false;  // Move to Constants!

  private CANSparkMax m_motor;
  private SparkMaxPIDController m_pidController;
  private RelativeEncoder m_encoder;
  public double kP, kI, kD, kIz, kFF, kMaxOutput, kMinOutput; //, rotationGoal;
  double p, i, d, iz, ff, max, min, rotationGoal = 0;
  int PeriodCount = 0;
  double SetPoint1 = 0.2;
  double SetPoint2 = 0.4;


  /** Creates a new HookArm. */
  public HookArm() {
    switch(Constants.THIS_ROBOT) {
      ...
      case Constants.PABLO_2022_FRC:
        ...
        m_motor.setSecondaryCurrentLimit(Constants.LOW_CURRENT_LIMIT);  // Amps (80 Max, primary SmartCurrentLimit only works for brushless)
        ...
        if (debugHookArm) {
          ...
          SmartDashboard.putNumber("Rotation Goal", rotationGoal);
        }
  ...
  public void setCurrentLimit(double current) {
    m_motor.setSecondaryCurrentLimit(current);  // Amps (80 Max, primary SmartCurrentLimit only works for brushless)
    return;
  }
  ...
  public void periodic() {
    ...
    switch(Constants.THIS_ROBOT) {
      ...
      case Constants.PABLO_2022_FRC:
        ...
        if (debugHookArm) {
          // periodic() should get called at 50 Hz (20 ms)
          // PeriodCount++;
          // if (PeriodCount > 100) {
          //   // Switch goal setpoint (for tuning)
          //   if (rotationGoal == SetPoint1) {
          //     rotationGoal = SetPoint2;
          //   } else {
          //     rotationGoal = SetPoint1;
          //   }
          //   PeriodCount = 0;
          // }
          ...
          // rotationGoal = SmartDashboard.getNumber("Rotation Goal", 0);
           ...
           */
          // m_pidController.setReference(rotationGoal, CANSparkMax.ControlType.kPosition);
          // m_motor.stopMotor();  // for debug
          
          SmartDashboard.putNumber("Arm Set Point", rotationGoal);
          SmartDashboard.putNumber("Arm Position Sensor", m_encoder.getPosition());
        }
      break;
    }
